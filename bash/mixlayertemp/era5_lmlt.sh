...
wrkDIR=/theia/data/brussel/vo/000/bvo00012/vsc10116/era5/proc/mixlayertemp

# set mask directory (lakecover)
maskDIR=/theia/data/brussel/vo/000/bvo00012/vsc10116/era5/lakemask

# set starting directory
inDIR=/theia/data/brussel/vo/000/bvo00012/data/dataset/era5/lakes/mixlayertemp/monthly
...
if [[ ! -e "era5_lmlt_monthly_${y1}_${y2}.nc" ]]; then

    files=()
    for y in $(seq $y1 $y2); do
        for f in era5_lmlt_monthly_*.nc; do
            if [[ "$f" == "era5_lmlt_monthly_${y}.nc" ]]; then
                files[${#files[@]}]="${f}"
            fi
        done
    done
    ...
    cdo -b F64 -O \
        mergetime \
        $(echo "${files[@]}") \
        "era5_lmlt_monthly_${y1}_${y2}.nc"
fi

# seasonal files to operate on
cdo -b F64 -O -L yearmean -selmon,12,1,2 era5_lmlt_monthly_${y1}_${y2}.nc $wrkDIR/lmlt_DJF_${y1}_${y2}.nc
cdo -b F64 -O -L yearmean -selmon,3/5 era5_lmlt_monthly_${y1}_${y2}.nc $wrkDIR/lmlt_MAM_${y1}_${y2}.nc
cdo -b F64 -O -L yearmean -selmon,6/8 era5_lmlt_monthly_${y1}_${y2}.nc $wrkDIR/lmlt_JJA_${y1}_${y2}.nc
cdo -b F64 -O -L yearmean -selmon,9/11 era5_lmlt_monthly_${y1}_${y2}.nc $wrkDIR/lmlt_SON_${y1}_${y2}.nc


# global annual mean time series
cdo -b F64 -O -L fldmean -yearmonmean era5_lmlt_monthly_${y1}_${y2}.nc $outDIR/era5_lmlt_global_annual_fldmean_${y1}_${y2}.nc


for season in "${seasons[@]}"; do

    # global mean time series for seasons
    cdo -b F64 -O fldmean $wrkDIR/lmlt_${season}_${y1}_${y2}.nc $outDIR/era5_lmlt_${season}_global_fldmean_${y1}_${y2}.nc

    # temporal mean for seasonal average maps
    cdo -b F64 -O timmean $wrkDIR/lmlt_${season}_${y1}_${y2}.nc $outDIR/era5_lmlt_${season}_timmean_${y1}_${y2}.nc
    ...
    cdo -b F64 -O -L sub $wrkDIR/lmlt_${season}_10year_end.nc $wrkDIR/lmlt_${season}_10year_start.nc $outDIR/era5_lmlt_${season}_signal_${y1}_${y2}.nc
